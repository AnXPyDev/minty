...
Key.mouselog = function ():void {
    ...
    Mouse.x = (((MClient.x - pos.x) / vport.scale.x - vport.size.x / 2) / camera.scale.x + camera.pos.x);
    Mouse.y = (((MClient.y - pos.y) / vport.scale.y - vport.size.y / 2) / camera.scale.y + camera.pos.y); 
}
