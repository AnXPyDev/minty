...
function loop3SidedPoly(polygon:Polygon, screenSize:Vector, forPoint:(pos:Vector) => void) {
	...
	for(let i:number = 0; i < 2; i++) {
		sides.push([polygon.val[i], polygon.val[wrap_np(i+1, 0, 2)]])
	}
	...
	for(let i:number = 0; i < 3; i++) {
		let is = false;
		let current:Vector[] = [polygon.val[i],polygon.val[wrap_np(i+1, 0, 2)]];
		...
		for(let e:number = 0; e < 2; e++) {
			if((current[e].x == polygon.corner.min.x &&
			    current[wrap_np(e+1, 0, 1)].x == polygon.corner.max.x)) {
				
				byWhichValue = "x";
				is = true;
			} else if (
			    (current[e].y == polygon.corner.min.y &&
				current[wrap_np(e+1, 0, 1)].y == polygon.corner.max.y)) {
	...
	average[0] = avg([translatePos(sides[wrap_np(ixFullSide[0] + 1, 0, 2)][0], step, isInverted).y, 
					  translatePos(sides[wrap_np(ixFullSide[0] + 1, 0, 2)][1], step, isInverted).y]); 
	average[1] = avg([translatePos(sides[wrap_np(ixFullSide[0] + 2, 0, 2)][0], step, isInverted).y, 
					  translatePos(sides[wrap_np(ixFullSide[0] + 2, 0, 2)][1], step, isInverted).y]); 
	
	let succession = [wrap_np(ixFullSide[0] + 1, 0, 2), wrap_np(ixFullSide[1] + 2, 0, 2)];

	if(average[1] < average[0]) {
		succession = [wrap_np(ixFullSide[0] + 2, 0, 2), wrap_np(ixFullSide[1] + 1, 0, 2)];
	}
